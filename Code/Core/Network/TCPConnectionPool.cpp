...
bool TCPConnectionPool::Listen( uint16_t port )
{
    ...
    {
        TCPDEBUG( "Create socket failed (Listen) (error %i)\n", GetLastError() );
        return false;
    }
    ...
    if ( ret != 0 )
    {
        TCPDEBUG( "setsockopt failed (error %i)\n", GetLastError() );
        CloseSocket( sockfd );
        return false;
    }
...
const ConnectionInfo * TCPConnectionPool::Connect( uint32_t hostIP, uint16_t port, uint32_t timeout, void * userData )
{
    ...
    if ( sockfd == INVALID_SOCKET )
    {
        TCPDEBUG( "Create socket failed (Connect) (error %i)\n", GetLastError() );
        return nullptr;
    }
    ...
    if ( connect(sockfd, (struct sockaddr *)&destAddr, sizeof( destAddr ) ) != 0 )
    {
        ...
        if ( !WouldBlock() )
        {
            // connection initiation failed
                ...
                TCPDEBUG( "connect() failed (error %i) (host:%s port:%u)\n", GetLastError(), host.Get(), port );
            #endif
            CloseSocket( sockfd );
            return nullptr;
        }
    ...
    for ( ;; )
    {
        ...
        if ( selRet == SOCKET_ERROR )
        {
            // connection failed
                ...
                AStackString<> host;
                GetAddressAsString( hostIP, host );
                TCPDEBUG( "select() after connect() failed (error %i) (host:%s port:%u)\n", GetLastError(), host.Get(), port );
            #endif
            CloseSocket( sockfd );
            return nullptr;
        }
...
void TCPConnectionPool::ListenThreadFunction( ConnectionInfo * ci )
{
    ...
    while ( ci->m_ThreadQuitNotification == false )
    {
        ...
        {
            TCPDEBUG( "accept failed (error %i)\n", GetLastError() );
            break;
        }
...
bool TCPConnectionPool::DisableNagle( TCPSocket sockfd )
{
    ...
    if ( ret != 0 )
    {
        TCPDEBUG( "setsockopt TCP_NODELAY failed (error %i)\n", GetLastError() );
        CloseSocket( sockfd );
        return false;
    }
    return true;
...
bool TCPConnectionPool::SetBufferSizes( TCPSocket socket )
{
    ...
    // Receive Buffer
    {
        ...
        if ( ret != 0 )
        {
            TCPDEBUG( "setsockopt SO_RCVBUF failed (error %i)\n", GetLastError() );
            CloseSocket( socket );
            return false;
        }
    }
    ...
    // Send Buffer
    {
        ...
        if ( ret != 0 )
        {
            TCPDEBUG( "setsockopt SO_SNDBUF failed (error %i)\n", GetLastError() );
            CloseSocket( socket );
            return false;
        }
    }
