...
/*static*/ bool ProjectGeneratorBase::WriteIfDifferent( const char * generatorId, const AString & content, const AString & fileName )
{
    ...
    {
        FLOG_ERROR( "%s - Invalid path for '%s' (error %i)", generatorId, fileName.Get(), Env::GetLastErr() );
        return false;
    }
    ...
    {
        FLOG_ERROR( "%s - Failed to open '%s' for write (error %i)", generatorId, fileName.Get(), Env::GetLastErr() );
        return false;
    }
    if ( f.Write( content.Get(), content.GetLength() ) != content.GetLength() )
    {
        FLOG_ERROR( "%s - Error writing to '%s' (error %i)", generatorId, fileName.Get(), Env::GetLastErr() );
        return false;
    }
    f.Close();
