...
REFLECT_NODE_BEGIN( LinkerNode, Node, MetaName( "LinkerOutput" ) + MetaFile() )
    REFLECT( m_Linker, MetaFile() )
    REFLECT( m_LinkerOptions, MetaNone() )
    REFLECT( m_LinkerType, MetaOptional() )
    REFLECT( m_LinkerAllowResponseFile, MetaOptional() )
    REFLECT( m_LinkerForceResponseFile, MetaOptional() )
    REFLECT( m_Libraries, MetaFile() + MetaAllowNonFile() )
    REFLECT( m_Libraries2, MetaFile() + MetaAllowNonFile() + MetaOptional() )
    REFLECT( m_LinkerAssemblyResources, MetaOptional() + MetaFile() + MetaAllowNonFile( Node::OBJECT_LIST_NODE ) )
    REFLECT( m_LinkerLinkObjects, MetaOptional() )
    REFLECT( m_LinkerStampExe, MetaOptional() + MetaFile() )
    REFLECT( m_LinkerStampExeArgs, MetaOptional() )
