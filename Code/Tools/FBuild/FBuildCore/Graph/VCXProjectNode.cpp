...
bool VCXProjectNode::Save( const AString & content, const AString & fileName ) const
{
    ...
    {
        FLOG_ERROR( "VCXProject - Invalid path for '%s' (error %i)", fileName.Get(), Env::GetLastErr() );
        return false;
    }
    ...
    {
        FLOG_ERROR( "VCXProject - Failed to open '%s' for write (error %i)", fileName.Get(), Env::GetLastErr() );
        return false;
    }
    if ( f.Write( content.Get(), content.GetLength() ) != content.GetLength() )
    {
        FLOG_ERROR( "VCXProject - Error writing to '%s' (error %i)", fileName.Get(), Env::GetLastErr() );
        return false;
    }
    f.Close();
