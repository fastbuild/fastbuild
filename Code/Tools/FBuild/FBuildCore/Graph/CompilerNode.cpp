...
REFLECT_NODE_BEGIN( CompilerNode, Node, MetaNone() )
    REFLECT( m_Executable, MetaFile() )
    REFLECT( m_ExtraFiles, MetaOptional() + MetaFile() )
    REFLECT( m_CustomEnvironmentVariables, MetaOptional() )
    REFLECT( m_AllowDistribution, MetaOptional() )
    REFLECT( m_AllowCaching, MetaOptional() )
    REFLECT( m_AllowResponseFile, MetaOptional() )
    REFLECT( m_ForceResponseFile, MetaOptional() )
    REFLECT( m_VS2012EnumBugFix, MetaOptional() )
    REFLECT( m_ClangRewriteIncludes, MetaOptional() )
    REFLECT( m_ClangGCCUpdateXLanguageArg, MetaOptional() )
    REFLECT( m_ClangFixupUnity_Disable, MetaOptional() )
    REFLECT( m_ExecutableRootPath, MetaOptional() + MetaPath() )
    REFLECT( m_SimpleDistributionMode, MetaOptional() )
    REFLECT_RENAME( m_CompilerFamilyString, "CompilerFamily", MetaOptional() )
    REFLECT( m_Environment, MetaOptional() )
    REFLECT_RENAME( m_UseLightCache, "UseLightCache_Experimental", MetaOptional() )
    REFLECT_RENAME( m_UseRelativePaths, "UseRelativePaths_Experimental", MetaOptional() )
    REFLECT_RENAME( m_UseDeterministicPaths, "UseDeterministicPaths_Experimental", MetaOptional() )
    REFLECT_RENAME( m_SourceMapping, "SourceMapping_Experimental", MetaOptional() )
