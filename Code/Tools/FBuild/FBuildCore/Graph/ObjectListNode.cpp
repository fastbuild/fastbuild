...
REFLECT_NODE_BEGIN( ObjectListNode, Node, MetaNone() )
    ...
    REFLECT( m_CompilerOptionsDeoptimized, MetaOptional() )
    REFLECT( m_CompilerOutputPath, MetaOptional() + MetaPath() )
    REFLECT( m_CompilerOutputPrefix, MetaOptional() )
    REFLECT( m_CompilerOutputExtension, MetaOptional() )
    REFLECT( m_CompilerOutputKeepBaseExtension, MetaOptional() )
    REFLECT( m_CompilerInputAllowNoFiles, MetaOptional() )
    ...
    REFLECT( m_CompilerInputUnity, MetaOptional() )
    REFLECT( m_CompilerInputFiles, MetaOptional() + MetaFile() )
    REFLECT( m_CompilerInputFilesRoot, MetaOptional() + MetaPath() )
    REFLECT( m_CompilerInputObjectLists, MetaOptional() )
    REFLECT( m_CompilerForceUsing, MetaOptional() + MetaFile() )
    REFLECT( m_DeoptimizeWritableFiles, MetaOptional() )
    REFLECT( m_DeoptimizeWritableFilesWithToken, MetaOptional() )
    REFLECT( m_AllowDistribution, MetaOptional() )
    REFLECT( m_AllowCaching, MetaOptional() )
    REFLECT( m_Hidden, MetaOptional() )
    // Precompiled Headers
    REFLECT( m_PCHInputFile, MetaOptional() + MetaFile() )
    REFLECT( m_PCHOutputFile, MetaOptional() + MetaFile() )
    REFLECT( m_PCHOptions, MetaOptional() )
    // Preprocessor
    REFLECT( m_Preprocessor, MetaOptional() + MetaFile() + MetaAllowNonFile() )
    REFLECT( m_PreprocessorOptions, MetaOptional() )
    REFLECT_RENAME( m_PreBuildDependencyNames, "PreBuildDependencies", MetaOptional() + MetaFile() + MetaAllowNonFile() )
    REFLECT( m_ConcurrencyGroupName, MetaOptional() )
