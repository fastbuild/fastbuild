...
/*virtual*/ Node::BuildResult ObjectNode::DoBuildMSCL_NoCache( Job * job, bool useDeoptimization )
{
    ...
    if ( ( ch.GetResult() == 0 ) && !GetFlag( FLAG_WARNINGS_AS_ERRORS_MSVC ) )
    {
        if ( IsClangCl() )
        {
            HandleWarningsClangCl( job, GetName(), ch.GetErr().Get(), ch.GetErrSize() );
            HandleWarningsClangCl( job, GetName(), ch.GetOut().Get(), ch.GetOutSize() );
        }
        else
        {
    ...
    if ( flags & (ObjectNode::FLAG_MSVC | ObjectNode::FLAG_CLANG_CL) )
    {
        ...
        if ( !usingCLR && !usingPreprocessorOnly )
        {
            ...
            if ( (( flags & ObjectNode::FLAG_USING_PDB ) == 0) ||
                ( !( flags & ObjectNode::FLAG_CLANG_CL ) || !( flags & ObjectNode::FLAG_CREATING_PCH ) || !FBuild::Get().GetOptions().m_DisablePCHCachingWithClang ) )
            {
...
bool ObjectNode::BuildArgs( const Job * job, Args & fullArgs, Pass pass, bool useDeoptimization, bool showIncludes, bool finalize, const AString & overrideSrcFile ) const
{
    ...
    for ( size_t i = 0; i < numTokens; ++i )
    {
        ...
        if ( isMSVC || isClangCl ) // not sure if force usings are understood by clang-cl
        {
            found = token.Find( "%4" );
            if ( found )
            {
...
bool ObjectNode::BuildFinalOutput( Job * job, const Args & fullArgs ) const
{
    ...
    {
        // Handle warnings if not already a failure
        if ( ( ch.GetResult() == 0 ) && !GetFlag( FLAG_WARNINGS_AS_ERRORS_MSVC ))
        {
            if ( IsMSVC() )
            {
...
//------------------------------------------------------------------------------
void ObjectNode::HandleWarningsClangCl( Job * job, const AString & name, const char * data, uint32_t dataSize )
{
    constexpr const char * clangClWarningString = " warning:";
    return HandleWarnings(job, name, data, dataSize, clangClWarningString);
}
...
bool ObjectNode::CanUseResponseFile() const
{
        ...
        return ( GetFlag( FLAG_MSVC ) || GetFlag( FLAG_GCC ) || GetFlag( FLAG_SNC ) || GetFlag( FLAG_CLANG ) || GetFlag( FLAG_CLANG_CL ) || GetFlag( CODEWARRIOR_WII ) || GetFlag( GREENHILLS_WIIU ) );
    #else
        return false;
    #endif
}
