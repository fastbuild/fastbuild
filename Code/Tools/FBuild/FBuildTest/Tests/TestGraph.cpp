...
void TestGraph::TestNodeTypes() const
{
	...
	{
		Dependencies empty;
		Node * n = ng.CreateExecNode( AStackString<>( "dst" ), fn, fn, AStackString<>( "args" ), AStackString<>( "workingDir" ), 0, empty, false );
...
void TestGraph::TestExecNode() const
{
	...
	Dependencies empty;
	ExecNode * execNode = ng.CreateExecNode( dstFile, srcNode, exe, exeArgs, workingDir, 1, empty, false );
